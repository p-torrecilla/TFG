...
#df = pd.read_csv('malware-detection/data.csv')


# Filtrar las filas donde el valor en 'SizeOfOptionalHeader' es un string
#ids_con_string = df[df["SizeOfOptionalHeader"].apply(lambda x: isinstance(x, str))]['ID']

# Mostrar los IDs correspondientes
#print("IDs con 'SizeOfOptionalHeader' como string:")
#print(ids_con_string.tolist())
...
# Predict the data
predict_data = pd.read_csv('malware-detection/data.csv')#, header=0)
predict_data = predict_data[X.columns]

# Convert all values in predict_data to float
...
data_2["Prediction"] = prediction[:-2]
data_2.to_csv('RF_balanced_data_2.csv', index=False)
